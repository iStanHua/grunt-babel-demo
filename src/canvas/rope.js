(function (global, factory) {
        ...
        createClass(SmoothCurve, [{
            ...
            value: function draw(ctx) {
                ...
                if (this._stripe) {
                    ...
                    for (var _i = 0, _len = vision.length; _i < _len; _i++) {
                        ...
                        for (var _i2 = 2, vlen = vp.length; _i2 < vlen; _i2 += 2) {
                            ctx.strokeStyle = this.color[_i2 / 2 % 2];
                            if (_i2 !== 2) {
                                ctx.beginPath();
                                ctx.moveTo((vp[_i2 - 2] + vp[_i2]) / 2, (vp[_i2 - 1] + vp[_i2 + 1]) / 2);
                            }
                    ...
                    ctx.beginPath();
                    ctx.moveTo(points[0], points[1]);
                    for (var _i3 = 2, _len2 = points.length; _i3 < _len2; _i3 += 2) {
                        if (_i3 === points.length - 4) {
                            ctx.quadraticCurveTo(points[_i3], points[_i3 + 1], points[_i3 + 2], points[_i3 + 3]);
                        } else {
                            ctx.quadraticCurveTo(points[_i3], points[_i3 + 1], (points[_i3] + points[_i3 + 2]) / 2, (points[_i3 + 1] + points[_i3 + 3]) / 2);
                        }
                    }
                    ctx.stroke();

                    var vision = this.vision;
                    for (var _i4 = 0, _len3 = vision.length; _i4 < _len3; _i4++) {
                        ctx.globalAlpha = _i4 / this.visionMax * this.visionAlpha;
                        var _vp = vision[_i4];
                        ctx.beginPath();
                        ctx.moveTo(_vp[0], _vp[1]);
                        for (var _i5 = 2, _vlen = _vp.length; _i5 < _vlen; _i5 += 2) {
                            if (_i5 === points.length - 4) {
                                ctx.quadraticCurveTo(_vp[_i5], _vp[_i5 + 1], _vp[_i5 + 2], _vp[_i5 + 3]);
                            } else {
                                ctx.quadraticCurveTo(_vp[_i5], _vp[_i5 + 1], (_vp[_i5] + _vp[_i5 + 2]) / 2, (_vp[_i5 + 1] + _vp[_i5 + 3]) / 2);
                            }
                        }
                        ctx.stroke();
                    }
                ...
                if (this.debug) {
                    ...
                    ctx.globalAlpha = 0.4;
                    ctx.moveTo(points[0], points[1]);
                    for (var _i6 = 2, _len4 = points.length; _i6 < _len4; _i6 += 2) {
                        ctx.lineTo(points[_i6], points[_i6 + 1]);
                    }
                    ctx.stroke();
                }
     ...
     */
    var SproutCurve = function () {
        function SproutCurve(option) {
            classCallCheck(this, SproutCurve);
        ...
        createClass(SproutCurve, [{
            key: 'addSeed',
            value: function addSeed(x, y, vx, vy) {
                this.seeds.push({
                    x: x,
                    y: y,
                    vx: vx,
            ...
            value: function sprout() {
                ...
                var dx = 0,
                    dy = 0;
                for (var i = 0, len = this.seeds.length; i < len; i++) {
                    dx = dy = 0;
                    var seed = this.seeds[i];
                    var nv = p5.noise(this.noise.value + i * this.noise.interval) * Math.PI * 5;
            ...
            value: function draw(ctx) {
                ...
                for (var i = 0, _len = this.seeds.length; i < _len; i++) {
                ...
                if (len > 5) {
                    ...
                    ctx.moveTo(points[0], points[1]);
                    for (var _i = 2; _i < len; _i += 2) {
                        if (_i === points.length - 4) {
                            ctx.quadraticCurveTo(points[_i], points[_i + 1], points[_i + 2], points[_i + 3]);
                        } else {
                            ctx.quadraticCurveTo(points[_i], points[_i + 1], (points[_i] + points[_i + 2]) / 2, (points[_i + 1] + points[_i + 3]) / 2);
                        }
                    }
    ...
    var curvejs = {
        ...
        perlin: p5,
        color: color,
        SmoothCurve: SmoothCurve$1,
        SproutCurve: SproutCurve
    };
    ...
    var joint = new Joint(20, 15, false, new Vector2(200, 100));
    var points = [];

    joint.points.forEach(function (v) {
        points.push(v.x, v.y);
    });
    ...
    canvas.addEventListener('mousemove', function (evt) {
        ...
        joint.points.forEach(function (v, i) {
            points[i * 2] = v.x;
            points[i * 2 + 1] = v.y;
        });
    }, false);
